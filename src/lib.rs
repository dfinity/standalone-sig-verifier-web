...
pub fn verify_basic_sig_by_public_key(msg: &[u8], signature: &[u8], public_key: &[u8]) -> Result<(), String> {
    ...
    ic_sig_ver::verify_basic_sig_by_public_key(public_key.algorithm_id, msg, signature, &public_key.key).map_err(|e| e.to_string())
}
...
pub fn verify_canister_sig(
    ...
    ic_sig_ver::verify_canister_sig(message, signature, public_key, root_of_trust).map_err(|e| e.to_string())
}
...
pub fn verify_ic_signature(
    ...
    let (public_key, content_type) = ic_sig_ver::user_public_key_from_bytes(public_key).map_err(|e| e.to_string())?;
    let result = match content_type {
        IcCanisterSignatureAlgPublicKeyDer => {
            let root_of_trust = root_public_key_from_bytes(ic_root_public_key)?;
            ic_sig_ver::verify_canister_sig(message, signature, &public_key.key, root_of_trust)
        }
        _ => ic_sig_ver::verify_basic_sig_by_public_key(public_key.algorithm_id, message, signature, &public_key.key)
    };
    result.map_err(|e| e.to_string())
}

fn root_public_key_from_bytes(ic_root_public_key: &[u8]) -> Result<IcRootOfTrust, String> {
    let root_key_bytes: [u8; 96] = ic_root_public_key.clone().try_into().map_err(|_| format!("Invalid length of ic root public key: expected 96 bytes, got {}", ic_root_public_key.len()))?;
